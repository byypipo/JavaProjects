package com.sebatmedikal.gcm.server;

import static com.sebatmedikal.gcm.server.Constants.GCM_SEND_ENDPOINT;
import static com.sebatmedikal.gcm.server.Constants.JSON_CANONICAL_IDS;
import static com.sebatmedikal.gcm.server.Constants.JSON_ERROR;
import static com.sebatmedikal.gcm.server.Constants.JSON_FAILURE;
import static com.sebatmedikal.gcm.server.Constants.JSON_MESSAGE_ID;
import static com.sebatmedikal.gcm.server.Constants.JSON_MULTICAST_ID;
import static com.sebatmedikal.gcm.server.Constants.JSON_NOTIFICATION;
import static com.sebatmedikal.gcm.server.Constants.JSON_PAYLOAD;
import static com.sebatmedikal.gcm.server.Constants.JSON_REGISTRATION_IDS;
import static com.sebatmedikal.gcm.server.Constants.JSON_RESULTS;
import static com.sebatmedikal.gcm.server.Constants.JSON_SUCCESS;
import static com.sebatmedikal.gcm.server.Constants.PARAM_COLLAPSE_KEY;
import static com.sebatmedikal.gcm.server.Constants.PARAM_DELAY_WHILE_IDLE;
import static com.sebatmedikal.gcm.server.Constants.PARAM_PAYLOAD_PREFIX;
import static com.sebatmedikal.gcm.server.Constants.PARAM_REGISTRATION_ID;
import static com.sebatmedikal.gcm.server.Constants.PARAM_TIME_TO_LIVE;
import static com.sebatmedikal.gcm.server.Constants.TOKEN_CANONICAL_REG_ID;
import static com.sebatmedikal.gcm.server.Constants.TOKEN_ERROR;
import static com.sebatmedikal.gcm.server.Constants.TOKEN_MESSAGE_ID;
...
import com.sebatmedikal.gcm.server.Result.Builder;
...
public class Sender {
	...
	public Result send(Message message, String registrationId, int retries) throws IOException {
		...
		do {
			attempt++;
			LogUtil.logMessage(getClass(), "Attempt #" + attempt + " to send message " + message + " to regIds " + registrationId);
	...
	public Result sendNoRetry(Message message, String registrationId) throws IOException {
		...
		LogUtil.logMessage(getClass(), "Request body: " + requestBody);
		HttpURLConnection conn = post(GCM_SEND_ENDPOINT, requestBody);
		int status = conn.getResponseCode();
		if (status == 503) {
			LogUtil.logMessage(getClass(), "GCM service is unavailable");
			return null;
		}
		if (status != 200) {
				...
				if (token.equals(TOKEN_MESSAGE_ID)) {
					...
					if (line != null) {
						...
						if (token.equals(TOKEN_CANONICAL_REG_ID)) {
							...
							LogUtil.logMessage(getClass(), Level.WARNING, "Received invalid second line from GCM: " + line);
						}
					}

					Result result = builder.build();
					LogUtil.logMessage(getClass(), "Message created succesfully (" + result + ")");
					return result;
	...
	public MulticastResult send(Message message, List<String> regIds, int retries) throws IOException {
		...
		do {
			attempt++;
			LogUtil.logMessage(getClass(), "Attempt #" + attempt + " to send message " + message + " to regIds " + unsentRegIds);
			multicastResult = sendNoRetry(message, unsentRegIds);
			long multicastId = multicastResult.getMulticastId();
			LogUtil.logMessage(getClass(), "multicast_id on attempt # " + attempt + ": " + multicastId);
	...
	public MulticastResult sendNoRetry(Message message, List<String> registrationIds) throws IOException {
		...
		setJsonField(jsonRequest, JSON_NOTIFICATION, message.getNotification());
		jsonRequest.put(JSON_REGISTRATION_IDS, registrationIds);

		// jsonRequest.put("notification", "\"title\": \"byypipo
		// TITLE1\",\"body\": \"byypipo BODY\"");

		// JSONObject notification = new JSONObject();
		// notification.put("title", "byypipo TITLE");
		// notification.put("body", "byypipo BODY");
		...
		LogUtil.logMessage(getClass(), "JSON request: " + requestBody);
		HttpURLConnection conn = post(GCM_SEND_ENDPOINT, "application/json", requestBody);
		int status = conn.getResponseCode();
		String responseBody;

		LogUtil.logMessage(getClass(), "status: " + status);
		if (status != 200) {
			responseBody = getString(conn.getErrorStream());
			LogUtil.logMessage(getClass(), "JSON error response: " + responseBody);
			throw new InvalidRequestException(status, responseBody);
		}
		responseBody = getString(conn.getInputStream());
		LogUtil.logMessage(getClass(), "JSON response: " + responseBody);
	...
	private IOException newIoException(String responseBody, Exception e) {
		...
		LogUtil.logMessage(getClass(), Level.WARNING, msg + " Exception: " + e.getMessage());
	...
	protected HttpURLConnection post(String url, String contentType, String body) throws IOException {
		...
		if (!url.startsWith("https://")) {
			LogUtil.logMessage(getClass(), Level.WARNING, "URL does not use https: " + url);
		}
		LogUtil.logMessage(getClass(), "Sending POST to " + url);
		LogUtil.logMessage(getClass(), "POST body: " + body);
