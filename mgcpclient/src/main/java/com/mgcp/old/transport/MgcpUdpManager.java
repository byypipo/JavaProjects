...
public class MgcpUdpManager extends Thread {
	...
	public MgcpUdpManager(MgcpConnectionManager mgcpConnectionManager) {
		try {
			serverSocket = new DatagramSocket(MgcpSharedParams.mediaServerPort, InetAddress.getByName(MgcpSharedParams.localAddress));
	...
	public void run() {
		try {
			...
			while (true) {
				try {
					receivePacket = new DatagramPacket(new byte[MgcpSharedParams.MAXPACKETLENGTH], MgcpSharedParams.MAXPACKETLENGTH);
	...
	public void sendMessage(String message) {
		try {
			...
			DatagramPacket sendPacket = new DatagramPacket(sendData, sendData.length, InetAddress.getByName(MgcpSharedParams.mediaServerAddress), MgcpSharedParams.mediaServerPort);
